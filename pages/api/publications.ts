...
import { getWebsitePublicationsPage, getPublicationCategoryList } from '@graphql/Query';
import { domainNameBasedOnEnv } from '@utils/helpers';
import {
    WebsitePublicationsQueryVariables,
    WebsitePublicationsQuery,
    WebsitePublicationCategoryListQueryVariables,
...
async function getDomainData(req: NextApiRequest, res: NextApiResponse) {
    ...
    const result = await client.query<
        ...
        WebsitePublicationCategoryListQueryVariables
    >({
        query: getPublicationCategoryList,
        variables: {
            domain_name: finalhost as string,
            orgId: Number(response.data?.website?.org?.pk),
        },
    });
    ...
    const { publicationCategoryList } = result.data;
    // console.log('server', websiteAboutPage);
    res.status(200).json({ publicationCategoryList });
}
