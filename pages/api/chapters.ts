...
import { getWebsitePublicationsPage, getOrgSubDivisions } from '@graphql/Query';
import { domainNameBasedOnEnv } from '@utils/helpers';
import {
    WebsitePublicationsQueryVariables,
    WebsitePublicationsQuery,
    OrgSubDivisionQueryQueryVariables,
    OrgSubDivisionQueryQuery,
} from '@gentypes/index';
...
async function getDomainData(req: NextApiRequest, res: NextApiResponse) {
    ...
    const result = await client.query<OrgSubDivisionQueryQuery, OrgSubDivisionQueryQueryVariables>({
        query: getOrgSubDivisions,
    ...
    const { orgSubDivisionList } = result.data;
    // console.log('server', websiteAboutPage);
    res.status(200).json({ orgSubDivisionList });
}
